...
namespace CS_CRUD_SQLServer
{
    public partial class Login : Form
    {
        ...
        private void btn_Login_Click(object sender, EventArgs e)
        {
            ...
            while (reader.Read())
            {
                //ASSIGN Admin LEVEL TO LOCAL VAR
                CSVar.AdminLevel = (int)reader[0];

                //CHECK IF LOGGED SUCCESSFULY
                if (CSVar.AdminLevel > 0)
                {
                    CSTool.infoMessage("Successfully logged in.");
                    //SHOW CPANEL if a valid Admin
                    if (CSULevel.Admin() || CSULevel.Support() || CSULevel.Admin())
                    {
                        //SHOW CPANEL
                ...
                {
                    CSTool.errorMessage("Incorrect Adminname or password.");
                }
            }
        ...
        private void txt_loginID_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                CSTool.errorMessage("Max Adminname characters is " + CSTool.maxLoginChars);
