...
namespace CS_CRUD_SQLServer
{
    public partial class Login : Form
    {
        ...
        private void btn_Login_Click(object sender, EventArgs e)
        {
            ...
            while (reader.Read())
            {
                //ASSIGN Manager LEVEL TO LOCAL VAR
                CSVar.ManagerLevel = (int)reader[0];

                //CHECK IF LOGGED SUCCESSFULY
                if (CSVar.ManagerLevel > 0)
                {
                    CSTool.infoMessage("Successfully logged in.");
                    //SHOW CPANEL if a valid Manager
                    if (CSULevel.Manager() || CSULevel.Support() || CSULevel.Admin())
                    {
                        //SHOW CPANEL
                ...
                {
                    CSTool.errorMessage("Incorrect Managername or password.");
                }
            }
        ...
        private void txt_loginID_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                CSTool.errorMessage("Max Managername characters is " + CSTool.maxLoginChars);
